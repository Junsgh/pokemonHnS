...
static void Task_NewGameBirchSpeech_ChallengesSoundRight(u8);
static void Task_NewGameBirchSpeech_WaitToSlidePlatformAway2(u8);
static void Task_NewGameBirchSpeech_ConfirmSlidePlatformAway2(u8);
...
static void Task_NewGameBirchSpeech_ReturnFromChallenges(u8);
...
static void CB2_NewGameBirchSpeech_ReturnFromTxRandomizerChallengesOptions(void)
{
    ...
    taskId = CreateTask(Task_NewGameBirchSpeech_ReturnFromChallenges, 0);
...
static void Task_NewGameBirchSpeech_ChallengesSoundRight(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Birch_ChallengesSoundRight);
    AddTextPrinterForMessage(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_WaitToSlidePlatformAway2;
}

static void Task_NewGameBirchSpeech_WaitToSlidePlatformAway2(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        //NewGameBirchSpeech_ShowChallengeMenu();
        gTasks[taskId].func = Task_NewGameBirchSpeech_ConfirmSlidePlatformAway2;
    }
}

static void Task_NewGameBirchSpeech_ConfirmSlidePlatformAway2(u8 taskId)
{
    if ((JOY_NEW(A_BUTTON)) || (JOY_NEW(B_BUTTON)))
    {
        NewGameBirchSpeech_StartFadeOutTarget1InTarget2(taskId, 2);
...
static void Task_NewGameBirchSpeech_ReturnFromChallenges(u8 taskId)
{
    if (gTasks[taskId].tTimer-- <= 0)
    {
        NewGameBirchSpeech_ShowDialogueWindow(0, 1);
        gTasks[taskId].func = Task_NewGameBirchSpeech_ChallengesSoundRight;
    }
}
